document.addEventListener('DOMContentLoaded', () => {
    ...
    function animateCapture(fromPit, oppositePit, targetMancala, onComplete) {
        ...
        // Clear pits in the board array first
        board[fromPit] = 0;
        board[oppositePit] = 0;
    ...
    function animateSow(stonesLeft, currentPit) {
        if (stonesLeft === 0) {
            animatedStones = []; // Clear remaining stones if any
            endTurn(currentPit);
            return;
        }
    ...
    function endTurn(lastPit) {
        ...
        afterTurnLogic(); // No capture, proceed
    }
    
    function checkGameOver() {
        if (gameOver) return true; // Prevent re-triggering
